...
def selectTbBank():
    query = "select bank_srl, bank_nm, manager, phone, email, use_fg, addr, addr_detail, zipcode, created_at, ci_image_path, admin_id from tb_bank order by bank_srl;"
    result = selectSql(query)
    parsingTbBank(result)

## targetDB에서 tb_branch 데이터 가져오기
def selectTbBranch():
    query = "select branch_srl, bank_srl, branch_nm, manager, phone, email, use_fg, addr, addr_detail, zipcode, created_at, admin_id from tb_branch order by branch_srl;"
    result = selectSql(query)
...
def parsingTbBank(result) :
    ...
    for data in result:
        param['idx'] = data[0]
        param['name'] = data[1]
        param['manager'] = data[2]
        param['phone'] = data[3]
        param['email'] = data[4]
        param['is_used'] = data[5]
        ...
        if (data[6] == None) or (data[6] == ''):
            ...
            for address in data[6].split(' ')[2:] :
                ...
                param['address_3'] = address3 + " " + data[7]
        ...
        param['ci_image_path'] = data[10]       
        param['admin_id'] = data[11]
        insertBankToClient(param)     

def insertBankToClient(param):
    query = "INSERT INTO tb_client (idx, name, manager, phone, email, is_used, address_1, address_2, address_3, zipcode, created_at, ci_image_path, admin_id) values ({}, '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}', '{}');"
...
def parsingTbBranch(result):
    ...
    for data in result:
        ...
        param['name'] = data[2]
        param['manager'] = data[3]
        param['phone'] = data[4]
        param['email'] = data[5]
        param['is_used'] = data[6]
        ...
        if (data[7] == None) or (data[7] == ''):
            ...
            address3 = ""
            address_0 = data[7].split(" ")
            param['address_1'] = address_0[0]
            param['address_2'] = address_0[1]
            for address in data[7].split(" ")[2:] :
                address3 += (address + " ")
            param['address_3'] = address3 if data[8] == None else address3 + str(data[8])

        param['zipcode'] = data[9]

        # 빈 데이터 처리
        param['created_at'] = ''
        if (data[10] == None) or (data[10] == ''):
            param['created_at'] = '1000-10-10'
        else :
            param['created_at'] = data[10]
        
        param['admin_id'] = data[11]
        insertBranchToClient(param)

def insertBranchToClient(param):
    query = "INSERT INTO tb_client (idx, parent_idx, name, manager, phone, email, is_used, address_1, address_2, address_3, zipcode, created_at, admin_id) values ({}, {}, '{}', '{}', '{}', '{}', {}, '{}', '{}', '{}', '{}', '{}', '{}')"
    query = query.format(param['idx'],
                        param['parent_idx'],
                        param['name'],
                        param['manager'],
